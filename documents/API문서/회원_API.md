...
- 인증이 필요하지 않습니다 (누구나 사용 가능).
- 이메일이 사용 가능하면 `true`, 중복이면 `false`를 반환합니다.
- 서버 단에서 `@Email` 제약을 통해 형식을 검증합니다.

### Query Parameters

| 파라미터명 | 타입 | 필수 | 설명 | 제약조건 |
|-----------|------|------|------|----------|
| email | String | 예 | 확인할 이메일 | 이메일 형식 (`@Email`), URL 쿼리 파라미터 |
...
**에러 메시지 예시:**
- `"Required request parameter 'email' for method parameter type String is not present"` - email 파라미터가 없을 때
- `"must be a well-formed email address"` - 이메일 형식이 올바르지 않을 때 (`@Email` 검증)
...
1. **인증 불필요**: 누구나 사용할 수 있는 오픈 API입니다.
2. **URL 인코딩**: 이메일에 `+` 등 특수 문자가 포함되면 URL 인코딩이 필요합니다.
3. **소프트 삭제 포함**: 탈퇴한 사용자의 이메일도 중복으로 간주됩니다.
...
- 인증이 필요하지 않습니다 (누구나 사용 가능).
- 닉네임이 사용 가능하면 `true`, 중복이면 `false`를 반환합니다.

### Query Parameters

| 파라미터명 | 타입 | 필수 | 설명 | 제약조건 |
|-----------|------|------|------|----------|
| nickname | String | 예 | 확인할 닉네임 | URL 쿼리 파라미터 |

### Request 예시

```
GET /api/auth/nickname/check?nickname=길동이
```
...
**에러 메시지 예시:**
- `"Required parameter 'nickname' is not present."` - nickname 파라미터가 없을 때
...
1. **인증 불필요**: 이 API는 인증이 필요하지 않습니다. 누구나 사용할 수 있습니다.
2. **URL 인코딩**: 닉네임에 특수문자나 한글이 포함된 경우 URL 인코딩을 해야 합니다.
3. **실시간 확인**: 회원가입 전에 실시간으로 닉네임 중복 여부를 확인할 수 있습니다.
4. **소프트 삭제**: 탈퇴한 사용자의 닉네임도 중복으로 간주됩니다 (소프트 삭제된 사용자 포함).
